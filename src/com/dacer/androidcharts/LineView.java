...
public class LineView extends View {
    ...
    private int topLineLength = MyUtils.dip2px(getContext(), 100);; // | | ←this
                                                                   //-+-+-
    private int sideLineLength = MyUtils.dip2px(getContext(),45)/3*2;// --+--+--+--+--+--+--
                                                                     //  ↑this
    private int backgroundGridWidth = MyUtils.dip2px(getContext(),60);
     ...
     private void drawBackgroundLines(Canvas canvas){
      ...
      if(bottomTextList != null){
    	  for(int i=0;i<bottomTextList.size();i++){
    		  String [] data = {"现在","明天","后天","大后天",""};
    		  canvas.drawText(data[i], sideLineLength+backgroundGridWidth*i, mViewHeight-bottomTextDescent, bottomTextPaint);
    	  }
      }
